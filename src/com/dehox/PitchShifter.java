...
public class PitchShifter{
	...
	public float[] process(float[] input) {
		...
		for(int i = 0 ; i<size ; i++){
			...
			input[i] = window * input[i];
		}
		//Fourier transform the audio 
		fft.forwardTransform(input);
		//Calculate the magnitudes and phase information. 
		fft.powerAndPhaseFromFFT(input, currentMagnitudes, currentPhase);
		...
		float[] audioBuffer = new float[stepSize];
		System.arraycopy(outputAccumulator, 0, input,0, stepSize);
